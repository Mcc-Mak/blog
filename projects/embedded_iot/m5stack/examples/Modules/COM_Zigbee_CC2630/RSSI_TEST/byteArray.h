...
public:
    ...
    explicit byteArray(const std::initializer_list<uint8_t> args){ this->initializer(args);}
    ~byteArray();
    byteArray &initializer(const std::initializer_list<uint8_t> args)
    {
        this->_ptr = (uint8_t*)realloc(this->_ptr,sizeof(uint8_t) * args.size());
        this->_length = 0;
    ...
    template<class Stream>void printfArray(Stream *uartPtr)
    {

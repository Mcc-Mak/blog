...
"""Stock price prediction and technical analysis for a Streamlit trading bot."""
...
# --- finance algorithms (EMA, RSI, Bollinger Bands, trading logic) ---
                ...
                if decision == 0:
                    ...
                    if (data["Close"][i] > data["BOLL_UB"][i] or (data["MACD"][i] < data["Signal"][i] and data["SMA"][i] <= data["SMA"][i - 1])):  # or (data["Close"][i]<data["SMA"][i]+data["BOLL_STD"][i] and data["SMA"][i-1]>data["SMA"][i]):
                        ...
                        if data["Close"][i] >= buy and data["Close"][i - 1] > data["Close"][i]:
                            ...
                            add_point = 2
                            buy = 0
                    if buy != 0:  # if the stock is not sold and it is below buying price, cut the loss
...
# --- data preprocessor: enrich df with indicators for the ML model ---
class DataPreprocessor(Finance_Algorithms):
    ...
    def get_format_data(self, data):
        # adds gains = close_today - close_yesterday, EMA, MACD, RSI, Bollinger Band
        # supports data format from yfinance
        # if self.data!= []:
        #     return self.data
        EMA = []  # 12 days EMA
        EMA_26 = []  # 26 days EMA
        ...
        data["EMA_12"] = EMA
        data["EMA_26"] = EMA_26
        data["MACD"] = data["EMA_26"] - data["EMA_12"]  # no absolute => MACD can be negative
        ...
        # ------- normalization (kept as commented reference) -------
